  ...
  return (
    <div className='space-y-4'>
        ...
        {data.length > 0 ? (
           <div className='flex flex-wrap gap-2'>
            {data.map((skill, index)=>(
              ...
              <button onClick={()=> removeSkill(index)} className='m1-1 hover:bg-blue-200 rounded-full p-0.5 transition-colors'>
                <X className='w-3 h-3'/>
              </button>
              </span>  
            ))}
        ...
        <div className='bg-blue-50 p-3 rounded-lg'>
            <p className='text-sm text-blue-800'> <strong>Tip:</strong> Add 8-12 relevent skills. Include both technical skills. (programming language, tools) and soft skills (leadership, communication).</p>
        </div>
    </div>
  )
}
